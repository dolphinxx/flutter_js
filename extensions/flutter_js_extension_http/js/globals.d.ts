...
type FlutterJSExtensionHttpOptions = {
  method?:HttpMethod;
  url:string;
...
type FlutterJSExtensionClientOptions = {
  /**
   * Document from http.dart:
   * 
...
type FlutterJSExtensionResponse = {
  headers:Record<string, string>;
  isRedirect: boolean;
  persistentConnection: boolean;
  reasonPhrase: string;
...
type FlutterJSExtensionErrorResponse = {
  statusCode: 0;
  reasonPhrase: string;
};
...
type FlutterJSExtensionHttp = {
  ...
  parseUri(raw:string): Promise<Uri>;
};
declare var FlutterJS:FlutterJSExtensionHttp;
